...
func buildTree(rows []string) string {
	...
	for _, row := range rows {
		...
		name = leftParts[0]
		currentProgram := programMap[name]

		if currentProgram == nil {
			newProgram := program{name: name, children: make([]*program, 0)}
			programMap[name] = &newProgram
			currentProgram = programMap[name]
		}

		currentProgram.weight, _ = strconv.Atoi(strings.TrimSuffix(leftParts[1], ")"))

		if len(parts) > 1 {
			right := parts[1]
			rightParts := strings.Split(right, ", ")

			for _, childname := range rightParts {
				childProgram := programMap[childname]

				if childProgram == nil {
					newchildProgram := program{name: childname}
					programMap[childname] = &newchildProgram
					childProgram = programMap[childname]
				}

				childProgram.parent = currentProgram
				currentProgram.children = append(currentProgram.children, childProgram)
			}
		}
	}

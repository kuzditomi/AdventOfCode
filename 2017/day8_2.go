...
var puzzle = `um inc -671 if lbf != 5
j inc 236 if umr > -6
fk inc -246 if j < 241
uy dec -404 if mmw <= 2
j inc 372 if gk >= -1
...
var operations = map[string]func(string, int){
	"inc": func(varname string, val int) { variables[varname] += val },
	"dec": func(varname string, val int) { variables[varname] -= val },
}

var conditions = map[string](func(int, int) bool){
...
func processRow(row string) Statement {
	...
	statement := Statement{
		...
		conditionLeft: parts[4],
		condition:     conditions[parts[5]],
	}

	statement.value, _ = strconv.Atoi(parts[2])
	statement.conditionRight, _ = strconv.Atoi(parts[6])
...
func main() {
	...
	for _, statement := range statements {
		currentVarname = statement.variableName

		if variables[currentVarname] > max {
			max = variables[currentVarname]
		}

		if statement.condition(variables[statement.conditionLeft], statement.conditionRight) {
			statement.operation(currentVarname, statement.value)
		}
	}

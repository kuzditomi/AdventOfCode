{
    ...
    const allKeys = Object.keys(allKeysHash).sort();
    let order = '';

    while (Object.keys(dependencies).length > 0) {
        for (let i = 0; i < allKeys.length; i++) {
            if (!dependencies[allKeys[i]] || dependencies[allKeys[i]].length === 0) {
                order += allKeys[i];

                Object
                    .keys(dependencies)
                    .forEach(k => dependencies[k] = dependencies[k].filter(d => d != allKeys[i]));

                delete dependencies[allKeys[i]];
                allKeys.splice(i, 1);

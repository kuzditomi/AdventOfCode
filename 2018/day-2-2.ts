{
    ...
    type uniqList = { [key: string]: number };
    const idLength = boxIds[0].length;

    for (let i = 0; i < idLength; i++) {
        const listWithoutCurrent = boxIds.map(id => id.substring(0, i) + id.substring(i + 1, idLength));

        const uniq = listWithoutCurrent
            .map((id) => {
                return { count: 1, id }
            })
            .reduce((a: uniqList, b) => {
